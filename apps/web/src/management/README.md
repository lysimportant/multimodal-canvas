...
- 应用的 `/auth/login`、`/auth/register`、`/auth/verify?email=...&purpose=...` 使用独立 `AuthenticationPage`；验证码复用本模块导出的 `VerificationForm`。初始化、邀请、密码重置和更换邮箱协议保持不变，邀请和重置要求设置密码，更换邮箱要求先登录。验证码只保存在输入框，注册确认后回工作台，改绑回账户安全，初始化回后台。
