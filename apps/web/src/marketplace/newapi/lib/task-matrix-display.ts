...
import { getTaskEnumFields, taskMatrixRowLabel, tryParseTaskMatrixConfig } from './task-expr';
...
export function getTaskPricingDisplayTiers(
  ...
  const fields = [
    ...new Set(previous.flatMap((tier) => tier.conditions.map((condition) => condition.field))),
  ].sort();
  let combinations: TaskTierCondition[][] = [[]];
  for (const field of fields) {
    const definition = schema[field];
    const values = definition?.type === 'boolean' ? ['false', 'true'] : definition?.enum;
    ...
    (combination) =>
      !previous.some((tier) =>
        tier.conditions.every((condition) =>
          combination.some(
            (value) => value.field === condition.field && value.value === condition.value,
          ),
        ),
      ),
  );
