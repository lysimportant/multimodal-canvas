 ...
 */
export function getConfiguredGroupRatio(groupRatio: Record<string, number>, group: string): number {
...
export function getDisplayGroupRatio(model: PricingModel, selectedGroup?: string): number {
  ...
  if (selectedGroup && selectedGroup !== FILTER_ALL && modelEnableGroups.includes(selectedGroup)) {
  ...
  for (const group of modelEnableGroups) {
    const ratio = groupRatio[group];
    if (typeof ratio === 'number' && Number.isFinite(ratio) && ratio < minRatio) {
      minRatio = ratio;
    }
  }
