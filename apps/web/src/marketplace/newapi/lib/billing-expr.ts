...
import { splitExpressionAtTopLevel, unwrapExpressionParens } from './billing-expression/structure';
...
export const BILLING_PRICING_VARS: BillingVar[] = BILLING_VARS.filter((v) => !v.isConditionOnly);
...
export type RequestCondition = TimeCondition | ParamHeaderCondition | EffortCondition;
...
function mapTokenTier(tier: TokenTier & { conditionText?: string }): ParsedTier {
  return {
    label: tier.label,
    ...
    ...Object.fromEntries(
      Object.entries(tier.prices).map(([key, price]) => [BILLING_VAR_KEY_TO_FIELD[key], price]),
    ),
  };
}
...
export function getCurrentTimePricingTiers(exprStr: string, now: Date): ParsedTier[] | null {
  return readTimeTokenPricing(exprStr, now)?.currentTiers.map(mapTokenTier) ?? null;
}
...
function tryParseTimeCondition(expr: string): RequestCondition | null {
  ...
  m = expr.match(/^(hour|minute|weekday|month|day)\("([^"]+)"\) (==|>=|<) ([\d.eE+-]+)$/);
  if (m) {
...
function tryParseTimeRangePair(lower: string, upper: string): RequestCondition | null {
...
function tryParseRequestConditions(conditionStr: string): RequestCondition[] | null {
...
export function requestRuleGroupsFromTrace(requestRules: RequestRuleTrace[]): RequestRuleGroup[] {
...
export function tryParseRequestRuleExpr(expr: string): RequestRuleGroup[] | null {
...
export function splitBillingExprAndRequestRules(expr: string): {
  ...
  const quantityParts = baseParts.filter((part) => unwrapOuterParens(part) === 'image_count');
...
export function combineBillingExpr(baseExpr: string, requestRuleExpr: string): string {
...
export function normalizeCondition(
  ...
  if (source === 'time') {
    ...
    const timeFunc: TimeFunc = isTimeFunc(timeCond?.timeFunc) ? timeCond.timeFunc : 'hour';
    ...
    return {
      ...
      rangeStart: timeCond?.rangeStart == null ? '' : String(timeCond.rangeStart),
...
function buildRequestConditionExpr(cond: RequestCondition): string {
  ...
  switch (normalized.mode) {
    case MATCH_EXISTS:
      return normalized.source === 'header' ? `${sourceExpr} != ""` : `${sourceExpr} != nil`;
    case MATCH_CONTAINS:
...
function buildRuleGroupFactor(group: RequestRuleGroup): string {
  ...
  const condExprs = (group.conditions || []).map(buildRequestConditionExpr).filter(Boolean);
