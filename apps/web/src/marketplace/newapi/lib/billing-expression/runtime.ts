...
export function timeInBillingZone(now: Date, zone: string): Record<TimeFunction, number> {
  ...
  try {
    ...
    if (resolved.toLowerCase() === timezone.toLowerCase() && resolved !== timezone) {
      throw new RangeError('timezone');
    }
  } catch {
    return {
...
class BillingRuntime {
  ...
  constructor(
    ...
    for (const [name, rawValue] of Object.entries(context.request?.headers ?? {})) {
  ...
  private evaluateNode(node: ExpressionNode): RuntimeValue {
    ...
    if (node.kind === 'conditional') {
      ...
      const ruleIndex = this.compiled.requestRules.findIndex((rule) => rule.node === node);
  ...
  private binary(node: Extract<ExpressionNode, { kind: 'binary' }>): RuntimeValue {
    if (node.operator === '&&') {
    ...
    if (typeof a === 'string' && typeof b === 'string' && node.operator === '+') {
      return { value: a + b };
    }
    if (
  ...
  private call(node: Extract<ExpressionNode, { kind: 'call' }>): RuntimeValue {
    ...
    switch (node.name) {
      ...
      case 'u': {
        ...
        const value = Object.hasOwn(this.context.usage, name) ? this.context.usage[name] : null;
        return { value };
      }
      case 'has': {
        ...
        return {
          value: source != null && substring !== '' && billingString(source).includes(substring),
        };
      }
      case 'min':
        return {
          value: Math.min(this.numeric(args[0]).value, this.numeric(args[1]).value),
        };
      case 'max':
        return {
          value: Math.max(this.numeric(args[0]).value, this.numeric(args[1]).value),
        };
      case 'abs':
...
export function evaluateBillingExpression(
  ...
  const compiled =
    typeof expression === 'string' ? compileBillingExpression(expression) : expression;
  ...
  try {
    ...
    return {
      ...
      ...(compiled.variables.has('image_count') ? { imageCount: context.imageCount ?? 1 } : {}),
      ...(runtime.fixedPrice !== undefined ? { fixedPrice: runtime.fixedPrice } : {}),
