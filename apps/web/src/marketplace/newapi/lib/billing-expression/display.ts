...
export function flattenBinary(node: ExpressionNode, operator: string): ExpressionNode[] {
  if (node.kind !== 'binary' || node.operator !== operator) return [node];
  return [...flattenBinary(node.left, operator), ...flattenBinary(node.right, operator)];
}
...
function nonnegativePriceLiteral(node: ExpressionNode): number | null {
  if (node.kind === 'literal' && typeof node.value === 'number' && node.value >= 0) {
    return node.value;
  }
  if (
    node.kind === 'unary' &&
...
function tokenTier(node: ExpressionNode, conditions: TokenTierCondition[]): TokenTier | null {
  if (
    node.kind !== 'call' ||
    node.name !== 'tier' ||
  ...
  for (const term of flattenBinary(node.args[1], '+')) {
    ...
    if (
      ...
      if (input.kind !== 'call' || input.name !== 'max' || input.args.length !== 2) {
        return null;
      }
      let remainder = input.args[0];
      if (remainder.kind === 'literal' && remainder.value === 0) {
        remainder = input.args[1];
      } else if (input.args[1].kind !== 'literal' || input.args[1].value !== 0) {
        return null;
      }
      if (
...
function timeTierBranches(
  ...
  return tiers.map((tier) => ({
    ...
    conditionText: [
      timeDescription,
      ...tier.conditions.map((condition) => `${condition.var} ${condition.op} ${condition.value}`),
    ].join(' && '),
  }));
}
...
function taskConditions(
  ...
  for (const term of flattenBinary(node, '&&')) {
    ...
    } else if (typeof value !== 'string' || !definition?.enum?.includes(value)) {
      return null;
    }
...
function taskTier(
  ...
  for (let term of flattenBinary(node.args[1], '+')) {
    ...
    const field = term.left.args[0].value;
    const definition = schema[field];
    if (definition?.type !== 'number' || !definition.unit || Object.hasOwn(unitPrices, field)) {
      return null;
    }
...
export function readTaskTierChain(
  ...
  let remaining = node;
  while (remaining.kind === 'conditional') {
    const conditions = taskConditions(remaining.condition, schema, includeBoolean);
    if (!conditions) return null;
