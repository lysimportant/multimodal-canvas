...
class BillingParser {
  ...
  private next(): Lexeme {
    ...
    while (/\s/.test(this.source[this.cursor] ?? '') && this.cursor < this.source.length) {
      this.cursor++;
    }
...
function checkExpressionTypes(ast: ExpressionNode): void {
  ...
  visitExpression(ast, (node) => {
    const requireType = (child: ExpressionNode, expected: ExpressionType): void => {
    ...
    if (node.kind === 'literal') {
      ...
      if (node.name === 'header') result = 'string';
        ...
        result = 'number';
      } else if (['fixed', 'min', 'max', 'abs', 'ceil', 'floor'].includes(node.name)) {
...
function validateFixedPricingTree(
  ...
  if (node.kind === 'binary' && node.operator === '*') {
    for (const [factor, pricing] of [
      ...
      if (rule && rule.multiplier >= 0 && !containsPricingMarker(rule.condition)) {
        validateFixedPricingTree(pricing, rules);
        return;
      }
    }
  }
  if (node.kind === 'call' && node.name === 'tier' && !containsPricingMarker(node.args[0])) {
  ...
  try {
    ...
    visitExpression(ast, (node) => {
      ...
      if (
        ...
        ![...probes.functions].some((name) => ['param', 'header', ...TIME_FUNCTIONS].includes(name))
      ) {
        return;
      }
      requestRules.push({
