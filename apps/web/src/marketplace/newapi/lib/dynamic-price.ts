...
export function getTaskUsageQuantityUnitLabelKey(unit: BillingUsageUnit | undefined): string {
...
export function getTaskUsagePriceUnitLabelKey(unit: BillingUsageUnit | undefined): string {
...
export function getDynamicPriceUnitLabelKey(entry: DynamicPriceEntry): string | null {
...
function isTaskPricingTier(tier: DynamicPricingTier): tier is ParsedTaskTier {
  return (
    Object.hasOwn(tier, 'unitPrices') && typeof (tier as ParsedTaskTier).unitPrices === 'object'
  );
}
...
export function isDynamicPricingModel(model: PricingModel): boolean {
  if (model.billing_plugin_variants?.length) {
    return model.billing_plugin_variants.some(
      (variant) => variant.billing_mode !== 'ratio' && Boolean(variant.billing_expr),
    );
  }
...
export function isUnconfiguredTaskUsageModel(model: PricingModel): boolean {
  ...
  return model.quota_type !== 1 && hasTaskUsageSchema(model) && !isDynamicPricingModel(model);
}

export function getTaskPricingUnit(model: PricingModel): BillingUsageUnit | null {
  const primaryField = getTaskNumberFields(model.billing_usage_schema)[0];
  return primaryField?.[1].unit ?? null;
}

export function getDynamicDisplayGroupRatio(model: PricingModel, selectedGroup?: string): number {
...
export function formatDynamicUnitPrice(
  ...
  const priceUSD = (valuePerMillionTokens * groupRatio) / TOKEN_UNIT_DIVISORS[options.tokenUnit];
...
export function getDynamicPricingTiers(model: PricingModel): DynamicPricingTier[] {
  if (model.billing_plugin_variants?.length) {
...
export function hasDynamicRequestRules(model: PricingModel): boolean {
  ...
  const { requestRuleExpr } = splitBillingExprAndRequestRules(model.billing_expr || '');
...
export function getDynamicPriceEntries(
  ...
  if (isTaskPricingTier(tier) && options.usageSchema) {
    const usageEntries: DynamicPriceEntry[] = getTaskNumberFields(options.usageSchema).flatMap(
      ([field, definition]) => {
  ...
  return BILLING_PRICING_VARS.flatMap((variable) => {
    ...
    if (variable.key === 'cr' && value !== 0 && value === (tier as ParsedTier).inputPrice) {
      return [];
    }
    ...
    return [
      {
        ...
        label:
          variable.key === 'cc' && typeof (tier as ParsedTier).cacheCreate1hPrice === 'number'
            ? 'Cache Creation (5m)'
            : variable.label,
        shortLabel:
          variable.key === 'cc' && typeof (tier as ParsedTier).cacheCreate1hPrice === 'number'
...
export function getDynamicPricingSummary(
  ...
  if (variants?.length) {
    const summaries = variants.flatMap((variant) => {
      const summary = getDynamicPricingSummary(withPluginPricing(model, variant), options);
    ...
    const ranges = new Map<string, { entry: DynamicPriceEntry; min: number; max: number }>();
    for (const providerEntries of [
      ...
      for (const entry of providerEntries) {
        ...
        if (
          !merged.unitLabel ||
          (typeof merged.unitLabel === 'object' && Object.keys(merged.unitLabel).length === 0)
        ) {
    ...
    const primaryKeys = new Set(
      summaries.flatMap((summary) =>
        summary.primaryEntries.slice(0, 1).map((entry) => `${entry.field}:${entry.unit}`),
      ),
    );
    const primary = entries.filter(
      (entry) =>
        (entry.unit !== 'request' && entry.unit !== 'image') || entry.field === 'modelPrice',
    );
    primary.sort((a, b) => Number(primaryKeys.has(b.key)) - Number(primaryKeys.has(a.key)));
  ...
  const baseExpression = splitBillingExprAndRequestRules(model.billing_expr || '').billingExpr;
  ...
  const tier = isTaskUsage ? (summaryTiers.at(-1) ?? null) : (summaryTiers[0] ?? null);
  ...
  return {
    ...
    primaryEntries: isTaskUsage
      ? entries.filter((entry) => entry.unit !== 'request' && entry.unit !== 'image')
      : entries.filter(
          (entry) =>
            entry.unit === 'request' ||
            entry.unit === 'image' ||
...
export function getCardExamplePrice(
  ...
  if (model.billing_plugin_variants?.length) {
    const variant = model.billing_plugin_variants.find((provider) => provider.billing_expr);
    return variant ? getCardExamplePrice(withPluginPricing(model, variant), options) : null;
  }
