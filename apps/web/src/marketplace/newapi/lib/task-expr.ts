...
*/
import type { BillingUsageExample, BillingUsageFieldSchema, BillingUsageSchema } from '../types';
import { parseTaskTiersFromExpr, splitBillingExprAndRequestRules } from './billing-expr';
...
export function createDefaultTaskVisualConfig(schema: BillingUsageSchema): TaskVisualConfig {
  return {
    tiers: [
      {
        label: 'base',
        conditions: [],
        constant: 0,
        unitPrices: Object.fromEntries(getTaskNumberFields(schema).map(([field]) => [field, 0])),
      },
    ],
  };
}

export function createDefaultTaskMatrixConfig(schema: BillingUsageSchema): TaskMatrixConfig {
  const unitPrices = Object.fromEntries(getTaskNumberFields(schema).map(([field]) => [field, 0]));
  return {
...
export function taskMatrixRowLabel(combination: Record<string, string>): string {
...
export function taskMatrixToTiers(
  ...
  const isUniform = config.rows.every(
    (row) =>
      ...
      numberFields.every(([field]) => row.unitPrices[field] === firstRow.unitPrices[field]),
  );
  if (isUniform) {
    return [
      {
        ...
        unitPrices: Object.fromEntries(
          numberFields.map(([field]) => [field, firstRow.unitPrices[field] ?? 0]),
        ),
      },
    ];
  }
  ...
  const rows = combinations.map((combination) => {
    ...
    const tier =
      tiers.find((candidate) =>
        candidate.conditions.every((condition) => combination[condition.field] === condition.value),
      ) ?? fallbackTier;
    return {
      combination,
...
export function normalizeTaskVisualConfig(
  ...
  const numberFields = new Set(getTaskNumberFields(schema).map(([field]) => field));
  const enumFields = new Map(
    getTaskEnumFields(schema).map(([field, definition]) => [field, definition.enum ?? []]),
  );
...
function generateTaskTierBody(
  ...
  for (const [field, definition] of numberFields) {
    ...
    if (definition.unit === 'token') {
      parts.push(`u(${JSON.stringify(field)}) * ${price} / ${TASK_TOKEN_PRICE_SCALE}`);
      continue;
    }
...
function generateTaskCondition(conditions: TaskVisualCondition[]): string {
  return conditions
    .map(
      (condition) => `u(${JSON.stringify(condition.field)}) == ${JSON.stringify(condition.value)}`,
    )
    .join(' && ');
}
