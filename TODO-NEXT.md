...
- `[~]` **P0-PROD-STARTUP-02 生产启动与安全边界**
  ...
  - 当前证据：代码和单元测试覆盖缺配置拒绝、认证、脱敏和限流边界；无 `DATABASE_URL` 的单机 API 会将 AI 凭据 AES-GCM 密文、历史版本和模型目录写入 Git 忽略的 `.data/ai-credentials.json`，首次保存生成同目录本机密钥文件，重启可恢复。已有凭据文件但本机密钥丢失或密文无法解密时会 fail-closed。
  - 未完成：真实生产模式启动、入口 TLS/反向代理部署验证、跨实例全局限流、跨进程历史凭据恢复、密钥轮换后的旧快照恢复和部署环境告警。
  - 已知风险：本地文件模式只支持单机单 API 进程，不能替代 PostgreSQL/BullMQ 的跨进程或多实例恢复；凭据使用单一加密密钥，尚无 key-id、旧密钥 fallback 或重加密迁移；Redis 限流故障时会退回进程内限流。
...
- `[!]` **P1-VIDEO-CONTRACT-04 New API 视频完整契约**
  - 当前证据：通用 Base URL 下的创建、查询 `done`、受保护 content 下载，以及隔离 Worker/MinIO/`asset_versions` 已验证。
  - 未完成：取消、Webhook/HMAC 签名、完整状态与错误字段、usage/计费字段、真实失败重试、生产任务恢复和供应商扣费幂等。
  - 依赖：平台提供并确认这些外部契约和可用测试凭据。
  - 验收：同一隔离项目完成创建、轮询或 Webhook、取消、失败、重试、下载、归档、去重和计费字段核对；未知字段不得臆造。

- `[~]` **P1-MEDIA-COVERAGE-05 图片、音频与参考输入**
  - 当前证据：真实最小图片请求返回 HTTP 200/base64；Provider 协议解析和 Mock 链路已有测试。
...
- `[~]` **P1-MULTIKEY-ROTATION-06 多 Key 轮换与历史恢复**
  - 当前证据：两把凭据在同一 DAG 中分别冻结 `credentialId/version`，文字+视频执行和重复幂等提交已通过；历史凭据精确读取有测试证据。
  - 未完成：真实密钥轮换、撤销后的排队任务策略、多进程恢复、生产队列恢复和供应商侧多 Key 扣费行为。
...
- `[~]` **P1-PROVIDER-ROLES-07 统一端口角色的真实映射**
  ...
  - 未完成：真实供应商对多角色、多参考图和跨媒体输入的字段契约；不能把角色静默降级或丢弃。
