...
- 本地接续仍需补齐受控 Key 轮换的新凭据版本及旧任务引用；不能用恢复授权抵消用户人工改 Key/期限。导入同事务、素材引用权限与版本、vip/auto 实际发送及带授权的 Worker SIGKILL 接管已补齐，见检查点；工作流导入不代替完整作品归属迁移。普通登录自动接入分组，不再以独立授权/撤销控件作为待办。
