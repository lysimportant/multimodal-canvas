...
- 3 个原请求 unknown 仍需上游管理员核实请求与收费，原令牌日志读取曾返回 401。用户最新明确不保留旧数据；旧实例、桌面归档和已确认的恢复副本均已删除，18 个旧项目、65 个素材不再等待归属转换。只能依据文档保留的原请求 ID 和上游日志继续核查，不重发原请求，不再声称本地备份可恢复。最新空库初始化与删除清单见[实施检查点](docs/newapi-account-implementation-checkpoint.md)。
