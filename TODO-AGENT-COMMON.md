...
| `deep_research`   | 多轮搜索、页内查找、跨来源比较、较高搜索预算 | 报告和调研             |
...
OpenAI、Anthropic、xAI/Grok、DeepSeek、智谱 GLM/ZLM 可以通过统一 Adapter 调用，但不代表能力完全相同。能力未知或不支持时，必须在付费请求前失败，例如：
...
预算达到阈值时，Run 必须暂停或失败并给出可诊断原因，不能继续隐式扣费。
...
| `packages/observability`  | Run、Step、Tool、Token、成本和失败追踪                  |
...
| 5    | `C4`、独立页面 `S4-S5`、画布 `N4`                      | 扩展多 Provider、Artifact 互通、深度研究、隔离浏览器和高风险动作审批                                                        | 高级能力在既有权限、预算、审计和幂等机制上灰度开放，不改变已完成 Run 的语义      |
