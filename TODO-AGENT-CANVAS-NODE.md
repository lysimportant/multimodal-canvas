...
- 本项目当前不向用户收费、不做本地额度扣减或账单结算；Agent 节点调用的模型、搜索和媒体费用由上游 API 供应商计算和结算。
- 节点只展示/记录供应商返回的 `usage`、状态、请求 ID 和可选费用字段；价格未知或费用字段缺失不阻塞节点功能实现。
- 步数、Token、输入大小、搜索次数、并发和超时等限制用于运行资源保护，不作为本地收费闸门。
...
`budget.maxCost` 仅保留为可选的上游估算/兼容字段，不用于本地收费、额度扣减或请求授权；实际执行限制由步数、Token、超时、输入大小和并发等运行资源边界承担。
...
不支持时显示具体角色、模型、Provider 和解决建议，不创建不符合契约的 ProviderJob。
...
| 深度研究   | 允许更多搜索和页面，但受节点运行资源上限限制        |
| 浏览器操作 | 首版隐藏或禁用；后续单独审批                        |

画布 Agent 默认 `offline`。开启联网时，节点面板必须显示域名过滤、最大搜索次数、外发文件策略和运行资源上限。
...
6. 运行资源限制：步数、Token、超时、输入大小和并发；不显示本地收费额度。
...
- 输入：上游节点、`@`资源、文件和版本。
- 上下文：分类 Token、估算/供应商实际 `usage`、压缩记录（不参与本地收费）。
...
- [ ] 输出端口按媒体类型和版本连接下游。
- [ ] 失败步骤单独重试，避免重复创建 Provider 请求；供应商费用由上游处理。
...
- [ ] Worker 重启、SSE 重放和重复队列消息不会重复创建 Provider 请求，并保留供应商返回的 `usage`。
...
6. Worker 重启、SSE 断线、失败重试和重复事件不会破坏画布或重复创建 Provider 请求；供应商 `usage` 可追溯。
...
- [ ] 节点级最大搜索次数、打开页面数和运行资源上限默认值；不设置本地金额预算。
