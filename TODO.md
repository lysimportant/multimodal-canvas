...
当前可执行条目计数（2026-08-25）：共 72 项；已完成 56 项，进行中 7 项，未开始 4 项，阻塞 1 项，暂缓 4 项（不含状态说明和需求模板）。新增条目时请同步更新本行。
...
- `[~]` Playwright/浏览器 smoke 已覆盖启动、节点创建、设置连接、上传、拖入、多参考连线、节点模型覆盖和结果展示（Chromium 6 tests）；真实模型切换仍待自动化
- `[x]` 已完成桌面和移动尺寸人工视觉验收；剪贴板支持版本化系统 Clipboard 与内存回退，解析校验已有单测，浏览器权限自动化仍待补齐
...
3. 补齐剪贴板权限场景的 Playwright 自动化，并在完整用户认证后接入下载 URL 会话鉴权。
