...
当前可执行条目计数（2026-08-25）：共 72 项；已完成 54 项，进行中 9 项，未开始 4 项，阻塞 1 项，暂缓 4 项（不含状态说明和需求模板）。新增条目时请同步更新本行。
...
- `[~]` Web Vitest 已覆盖画布文档转换、环路检测、连线校验、上传协议、剪贴板、快捷键、设置表单和画布编辑器交互（34 tests）；上传、拖入、多参考连线组件测试仍待补齐
- `[~]` Playwright/浏览器 smoke 已覆盖启动、节点创建和设置连接测试（Chromium 3 tests）；上传、拖入、多参考连线和真实模型切换仍待自动化
