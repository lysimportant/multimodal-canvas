...
当前可执行条目计数（2026-08-25）：共 70 项；已完成 53 项，进行中 9 项，未开始 4 项，阻塞 1 项，暂缓 3 项（不含状态说明和需求模板）。新增条目时请同步更新本行。
...
- `[x]` `pnpm lint`
- `[x]` `pnpm typecheck`
- `[x]` `pnpm test`（API 82 通过、1 跳过；domain 11、providers 4、worker 6、Web 30、observability 10；UI 无测试文件；无 `TEST_DATABASE_URL` 时 Prisma 集成测试安全跳过）
- `[x]` `pnpm build`
...
- `[~]` Bearer API token、Webhook HMAC、项目资源隔离和生产缺失令牌保护已完成；完整用户鉴权待接入
...
- `[~]` 画布平移、缩放、框选、多选、复制/粘贴、删除、撤销/重做、自动保存已实现；连线端口/环路/重复校验已抽出并完成纯逻辑测试，交互自动化测试和视觉验收仍待补齐；快捷键已避开表单控件，来源节点不再暴露输入端口
...
- `[~]` Web Vitest 已覆盖画布文档转换、环路检测、连线校验、上传协议、剪贴板、快捷键和设置表单纯逻辑，并新增 SettingsPanel React Testing Library 交互测试（30 tests）；Store、节点和连线组件测试仍待补齐
- `[~]` Playwright/浏览器验收已覆盖页面恢复、节点创建、撤销/重做、设置弹窗和多媒体节点；上传、拖入、多参考连线和真实模型切换仍待自动化
- `[~]` 已完成桌面和移动尺寸人工视觉验收；剪贴板自动化仍待补齐
...
3. 补齐 React Testing Library、Playwright 上传/拖入/多参考测试和剪贴板自动化。
