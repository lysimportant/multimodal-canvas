...
最后一个命令要求 Web 已运行；它只改写 API 请求目的端口，实际请求经过 Fastify、Prisma 和文件存储，邮箱使用内存替身。覆盖初始化、邀请 A/B、邮箱激活、创建项目、真实 mock 生成归档、运行中与完成后的头像/个人信息往返、首页返回工作台、按用户资源、管理员编辑/归档/恢复、禁用和退出。核对身份、节点尺寸、结果可见和生成/注销次数；测试报告明确区分合成邮件、实际 SMTP 握手和真实邮件送达。
