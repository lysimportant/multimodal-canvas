...
| 文字 | `/chat/completions`               | 没有修改 URL。把**回显正文 + 当前提示词**放进子节点请求，子节点编辑器不落提示词，再走 `/chat/completions`                                  |
| 音频 | `/audio/speech`                   | 没有修改 URL，也不能把音频当文本。子节点继承音色参数，请求用父提示词再 TTS，父节点音频保留                                                 |
| 视频 | 当前 `videoMode` 的已落地生成合同 | `video_edit` / `video_extend` 未开放，不猜 URL。有现成末帧图则子节点走已落地的首帧模式；否则用父提示词按当前模式再生成到新节点，父视频保留 |
...
- 子节点全新 ID，不带父节点产物字段，也不落父节点提示词、`promptDocument`、`resourceRefs`。
- 无提示词：两条按钮都禁用；「新节点」不建节点、不发请求。文字若读不出回显正文，同样不建节点。
- 「新节点」立刻 `POST /v1/nodes/{子节点}/runs`：请求体带上父节点当前提示词（文字再拼上回显正文）以及继承的模型/参数，但这些提示词不写进子节点 data。
...
4. 拷贝模型、参数；不拷贝提示词、`promptDocument`、`resourceRefs`。
5. 立刻对子节点 `POST /v1/nodes/{子节点}/runs`，请求体带上父节点当前提示词。
...
2. 新建文字子节点，继承模型、凭据、推理强度；不落提示词和 `resourceRefs`。
3. 请求体提示词 = 当前提示词 + 分隔 + 回显正文。`promptDocument` 在保留原提及块的前提下**追加一段纯文本**（回显），只放进 POST，不写子节点。不要先渲染再整段覆盖，以免丢掉 `@` 资源提及。
4. 父节点提示词和回显都不改；子节点编辑器保持空提示词。
...
1. 新建音频子节点。
2. 继承音色/格式/语速等 `parameters`、模型、凭据；不落提示词和 `resourceRefs`。
3. 不把父节点音频资产拷到子节点，也不新建「音频编辑」边。
4. 立刻对子节点跑 `/audio/speech`，请求体带上父节点当前提示词。
5. 父节点音频回显保留。
...
3. 拷贝模型、视频参数、凭据；不落提示词。
4. 立刻跑已落地的首帧/图生视频合同，请求体带上父节点当前提示词。
5. 父视频不动。
6. **这次点击里不抽帧**。默认末帧动作仍是 `none`，不为了分叉去新建派生资产。

**否则（没有末帧图）：**

1. 子节点继承当前 `videoMode`、模型、参数；不落提示词。
2. 不把父视频接到子节点：`text_to_video` 禁止参考；接成参考会改变模式。
3. 立刻按该模式再生成到新节点，请求体带上父节点当前提示词。常见情况是再走文生视频。
4. 父视频回显保留。
...
| 类型 | 继承                               | 新写入                                                                   | 入边                                                  |
| ---- | ---------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------- |
| 图片 | 模型、参数                         | 新 id、图生图 label、指向**当前回显**的 imageEditSource；提示词只进 POST | 仅新建 `input:imageEdit`                              |
| 文字 | 模型、推理强度                     | 新 id；拼好的 prompt / promptDocument 只进 POST                          | 不把父文字当 content 边重复接入（正文已进请求提示词） |
| 音频 | 音色参数、模型                     | 新 id；提示词只进 POST                                                   | 无音频编辑边                                          |
| 视频 | 模型、参数；无末帧时继承 videoMode | 新 id；有末帧时 videoMode=first_frame；提示词只进 POST                   | 有末帧则 `input:firstFrame`；否则不接父视频           |
...
- 图片复用修改图片的冻结与连线；当前提示词只放进立刻发出的 POST，不写子节点。悬浮栏「修改图片」调用同一函数。
- 文字在建节点前读回显正文；读失败整次回滚。拼接结果只进请求体。
