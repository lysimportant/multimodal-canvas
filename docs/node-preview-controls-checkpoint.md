...
- [x] 实现图片两次点击和媒体原始比例适配；使用实际快速编辑器 ID，Ctrl+A 全选不能绕过首击输入。
...
不修改数据库、持久化节点尺寸或公开 API。来源节点保持直接预览；有输入框的生成/转换节点先进入编辑，再允许点击图片预览，展开按钮仍可直接预览。预览小媒体保持原尺寸，大媒体按视口等比缩小；标题与控件保留必要空间。
