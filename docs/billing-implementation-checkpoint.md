...
2026-09-20：所有保存 Key 的模型按来源供普通用户选择，保存连接自动联动、历史连接全部同步，New API 价格直接沿用。独立实施状态、兼容和验证记录见[画布多连接检查点](canvas-connections-checkpoint.md)。本项不关闭 P2-03 的生产、付费 Provider、支付和运维验收。

## 凭据来源与已保存连接管理追加任务

- P1 目标：节点默认使用紧凑的凭据来源选择器，地址、Key 尾号和当前全局状态可辨认；每条保存连接可直接删除，无需先激活。删除前确认影响，失败保留连接，成功清理目录缓存和待绑定草稿，不静默切换其他 Key。
- 起点 `codex/generate-to-new-node @ 2fbf6e2`，上游 `origin/codex/generate-to-new-node`；Node `24.12.0`、pnpm `11.19.0`，项目依赖已存在。设置、凭据查询、模型管理基线共 74 项通过，日志 `connection-management-baseline.log`。
- 范围及回滚：复用现有删除 API，无数据库、持久格式或接口迁移。既有删除语义会移除同地址、同 Key 的所有可选版本与独立副本；当前全局匹配时清空活动连接，历史密文按冻结 id/version 保留。确认框提示模型绑定和默认配置需重选，不承诺上游撤销 Key 后仍可执行。回退本轮前端提交即可恢复旧入口，已确认删除的连接需用原 Key 重新添加；不会通过恢复旧库覆盖新账务。
- 只在隔离环境创建及删除本轮合成连接，不删除既有连接，不调用真实模型，不修改 New API、钱包或用户的 `docs/resource-input-compatibility.md`。生产上线及其他未完成项仍由 TODO P2-03 跟踪。
- 协作工具再次出现消息空载，新建代理达到线程上限；复用代理未能确认当前范围，已中断，主代理完成实现与整合。
- 实现：紧凑来源下拉框、独立连接列表和逐条删除确认，模型后台及同步弹窗增加管理入口。删除非活动连接保留地址/Key/超时草稿；移除待绑定选择、失效目录缓存并刷新模型管理及广场缓存。已删除项目引用保持失效，不再请求其目录；请求互斥和账户切换检查沿用现有机制。
- 定向验证：Web 设置/查询/模型管理阶段 77 项通过；最后新增来源取消分支已进入全量 Web 1,153 项通过。凭据内存、文件、Prisma 模拟和真实 PostgreSQL 同步 104 项通过，其中 12 项真实数据库，零跳过；覆盖删除不复活及冻结历史版本解密。日志 `connection-management-{web,api}.log`。
- PC Web：本机 API 13000 与 Vite 15173，1440×1000 检查独立页和画布弹窗，1280×720 检查连接列表。真实 API/数据库验证取消、直接删除非活动连接、刷新持久化、清理待绑定选择、项目失效引用和模型绑定保留。两个本轮合成 Key 已删除、合成模型已逻辑删除、项目已归档，既有设置及连接逐项比较不变；上游 POST、生成 POST、页面及控制台错误均为 0。截图及报告 `connection-management-*.png`、`connection-management-browser-result.json`。首次脚本在平台模型上架前保存项目默认被正确拒绝，调整夹具顺序后通过；没有放宽业务校验。
- 最终检查：`pnpm lint` 9/9、`pnpm typecheck` 15/15、`pnpm test` 15/15、`pnpm build` 9/9 及 `pnpm build:runtime` 全部通过。全量 3,230 项通过，122 skipped、另 5 pending 不计通过；未改动包复用 Turbo 缓存，Web 最终 77 文件、1,153 项重新执行通过。Web 保留既有大包提示。日志统一为 `.data/billing-implementation/connection-management-{lint,typecheck,test,build,runtime}.log`。
- 交付目标：`origin/codex/generate-to-new-node`，附注标签 `v2026.09.20-connection-management`；核验远端分支和标签解引用。原用户文档 SHA256 仍为 `56B2C9D2BFB09DCC56720769B9CE12AED4877A29090DEDFBADD2F1FC5B3AA2A7`，不纳入提交。生产验收和既有 P2-03 剩余项不因本轮前端交付关闭。
