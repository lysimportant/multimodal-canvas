...
> 2026-09-17 兼容策略更新：图片流程不再因缺少 `mentionMediaTypes` / `imageEdit` 声明而拦截，图片资源提及自动映射到 `edits`，明确不支持及输入边界仍拒绝。当前规则见 [图片引用与编辑兼容](image-reference-compatibility.md)。下文保留原实施步骤与历史验收，原有“未知能力拒绝”描述不再代表当前规则。
