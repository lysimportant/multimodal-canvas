...
- 包测试、API/Worker 定向测试和真实媒体验收分别记录，不互相替代；文本和图片通过不代表音频或视频已通过。
...
API/Worker 的显式视频配置接线已完成，对应两组定向测试分别为 58 passed、77 passed，构建通过。真实文本、图片已通过同一 origin 的 Worker/DB/S3 全链路，文本 requestId 的真实持久化也已验证。这些证据不外推到未测试模型、参数或媒体类型。
...
真实视频的两次调用分别经过一次性授权，均使用单数 `/v1/video/generations`，结果均为 HTTP 404：首次为 `grok-imagine-video-1.5 （按次）`，再次按用户纠正原样发送 `grok-imagine-video-1.5（按次）`。未自动重试或切换协议重发；这些结果不能证明模型不支持，也不能把 404 归因于名称中的空格。当前独立真实验收记录尚无复数接口的成功生成与归档证据，以 `docs/live-provider-acceptance.md` 的实测记录为准。

## 尚未验证与支持边界

1. 当前 Helunox/Sub2API 部署的复数视频接口、分组权限、输出和归档完整结果，以及指定模型实际接受的参数范围；公开路由已找到，但当前记录尚无本项目成功证据，不自动重试。
