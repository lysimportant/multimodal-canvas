...
- 当前尚未实现收费、额度扣减或账单结算；后续按下方执行方案补充 usage 展示、费用字段、钱包和账单对账。
- 后续本地钱包、模型广场、平台定价、冻结结算和 Provider 成本对账按 [本地计费与模型广场执行方案](docs/billing-and-model-marketplace-plan.md) 实施；在代码、迁移和验收完成前本项保持未完成。
